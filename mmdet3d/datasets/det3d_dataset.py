...
class Det3DDataset(BaseDataset):
    ...
    def __init__(self,
        ...
        if self.label_merge is not None:
            for ori_cls, tgt_cls in self.label_merge.items():
                ori_cls_index = self.METAINFO['classes'].index(ori_cls)
                tgt_cls_index = self.METAINFO['classes'].index(tgt_cls)
                self.label_mapping[ori_cls_index] = self.label_mapping[tgt_cls_index]
        ...
        if self.modality['use_lidar']:
            ...
            if 'lidar_sweeps' in info:
                for sweep in info['lidar_sweeps']:
                    file_suffix = sweep['lidar_points']['lidar_path'].split(os.sep)[-1].split('/')[-1]
