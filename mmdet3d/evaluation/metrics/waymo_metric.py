...
class WaymoMetric(KittiMetric):
    ...
    def waymo_evaluate(self,
        ...
        if metric == 'mAP':
            eval_str = f'/home/dzh/projects/ASNet/mmdet3d/evaluation/functional/waymo_utils/compute_detection_metrics_main {pklfile_prefix}.bin /home/dzh/dataset/detection3d/waymo/waymo_format/gt.bin'
            ...
            if True:
                ...
                src = f'{pklfile_prefix}.bin'
                shutil.copy(src, os.path.join('tmp', f'{current_time}_{os.path.basename(src)}'))  # 保存临时生成的预测bin文件
